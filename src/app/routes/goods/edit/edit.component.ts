...
export class GoodsEditComponent implements OnInit {
  ...
  constructor(
    ...
    categorySvc.getCategorySelectList().subscribe(res => {
      const categories = [];
      res.forEach(item => {
        categories.push(new ProductCategoryDto({
          id: item.value,
          name: item.text
        }));
      });
  ...
  addAttribute() {
    this.product.attributes.push(new ProductAttributeDto({
      id: 0,
      values: [],
      name: undefined
    }));
  ...
  attributeChange(e, i) {
    if (e) {
      ...
      this.product.attributes = this.product.attributes.filter(attribute => attribute.id !== this.product.attributes[i].id);
    }
    this.createSku();
  }
  ...
  getProductImages(files: { url: string, id: number }[]) {
    const pictures = [];
    files.forEach(file => {
      pictures.push(new ProductPictureDto({
        id: file.id,
        url: file.url
      }));
    });
