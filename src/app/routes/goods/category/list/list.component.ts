...
export class GoodsCategoryListComponent implements OnInit {
      ...
      title: '操作', buttons: [
        {
          text: '更新',
          click: (item: any) => {
            this.router.navigate(['/goods/category/edit', item.id]);
          },
        },
        {
          text: '删除',
  ...
  getData() {
    this.loading = true;
    this.categorySvc.getCategorys(this.q.name, this.q.parentCategoryId, this.q.sorting, this.q.maxResultCount, this.q.skipCount).subscribe(res => {
      console.log(res);
      this.loading = false;
      const items = [];
