...
export class AdvertListComponent {
  ...
  getData() {
    ...
    this.accountSvc.getAccounts(
      ...
      this.q.productId,
      this.q.sorting,
      this.q.maxResultCount, this.q.skipCount).subscribe(res => {
      this.loading = false;
      this.data = res;
    });
  }
  ...
  getUrl(data) {
    let url = '';
    if (data.channel === 20) {
      url = `http://ad.toutiao.com/openapi/audit/oauth.html?app_id=${abp.setting.values['Advert.ToutiaoAdsAppId']}&redirect_uri=${encodeURIComponent(location.href)}&state=${data.id}&scope={scope.UrlEncode()}`;
    } else {
      url = `https://developers.e.qq.com/oauth/authorize?client_id=${abp.setting.values['Advert.TenantAdsAppId']}&redirect_uri=${encodeURIComponent(location.href)}&state=${data.id}&scope=`;
    }
    return url;
  }
